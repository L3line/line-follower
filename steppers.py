...
class stepperDrive(object):
    ...
    def __init__(self, stepsPerRev=200, style=Adafruit_MotorHAT.INTERLEAVE, wheelRad=0.04, wheelBase=0.144):
    ...
    def driveBlocking(self, commands):
        for driveCmd in commands:
            if not self.interrupt.is_set():
                self.leftDrive = threading.Thread(target=self.stepper_worker, args=(self.leftMotor, driveCmd[0], driveCmd[2]))
                self.rightDrive = threading.Thread(target=self.stepper_worker, args=(self.rightMotor, driveCmd[1], driveCmd[2]))
                self.leftDrive.start()
                self.rightDrive.start()
                self.leftDrive.join()
                self.rightDrive.join()
