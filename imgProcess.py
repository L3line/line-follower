...
def imgProcess():
    ...
    image,contours,hierarchy = cv2.findContours(edges,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) #Find contours in canny image
    ...
    for cont in range(len(contours)):
        if len(contours[cont]) < 19: #remove any small contours (typically errors)
            contours_shorten.remove(contours[cont])
    
    contour_pertr = [] # perspective correction contours
    contours_pertrInts = [] 
    
    for cont3 in range(len(contours_shorten)): #iterate through contour list
        contours_shorten[cont3] = contours_shorten[cont3].reshape(-1,2)
        a = contours_shorten[cont3] # take each contour in term
        a = np.array([a])
        a = a.astype(float)
        contour_pertr.append(cv2.perspectiveTransform(a,h)) # apply perspective transform to each contour
